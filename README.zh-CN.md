...
- **开机自动启动**：主窗口、系统托盘、悬浮球三个右键菜单都能开关。写在当前用户的
  ...
  而不是无声失效。
- **在线更新**：启动后自动检查 GitHub 最新正式版，运行期间每 6 小时再后台复查一次——配合开机自启，
  程序连着挂几周也不会错过新版本。发现新版立即弹窗；选了“稍后”的那个版本 24 小时内不再打扰，
  但更新的版本随时会提示。也可从主窗口、系统托盘或悬浮球右键菜单手动检查。确认更新后会自动下载、
  核对文件大小与 SHA-256、替换原 EXE 并重启，整个过程不需要 GitHub Token。
- **原生窗口行为**：DWM 系统圆角、最小化/恢复动画、最小化到任务栏、隐藏到托盘；退出只在托盘菜单，
  避免误关。
- **持续置顶**：主窗口与悬浮球在切换应用、恢复窗口及其他置顶窗口遮挡后自动恢复层级，不抢输入焦点。
  隐藏或最小化后暂停维护；自己的右键菜单和更新对话框仍在上方。此功能适用于普通 Windows 桌面，
  无法覆盖 UAC 安全桌面、锁屏或独占全屏。
...
便携程序所在目录必须对当前 Windows 用户可写。**这一点在下载之前就会检测**：如果那个目录不接受写入，
更新提示会直接说明原因，并把按钮换成「打开下载页」，而不是让你下完整个安装包才失败。常见原因是
Windows 安全中心的「受控文件夹访问」拦截了对桌面或文档的写入、EXE 被放在 Program Files 等需要管理员
权限的目录，或杀毒软件阻止替换 .exe。把程序移到自己可写的普通文件夹即可恢复自动更新。

万一安装阶段仍然失败，原 EXE 不会被破坏（替换是先写临时文件再原子移动），程序会在提示后自动重新启动。
