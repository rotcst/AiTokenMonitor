...
- **액체 오브 모드**: 카드를 더블클릭하면 작은 오브로 접히며, 좌우 두 수조가 각각 Codex와 Claude로,
  수위가 각 서비스의 **남은 한도**입니다(가득 차면 100%, 바닥이면 0%; 5시간 창이 있으면 5시간, 없으면
  주간). 수면은 계속 일렁입니다. 오브를 더블클릭하면 다시 펼쳐집니다.
...
온라인 업데이트와 호환되는 Release를 만들 때는 태그와 프로젝트 버전을 맞추고(예: `v1.9.7`),
`AiTokenMonitor-1.9.7.exe`를 업로드하세요. 클라이언트는 GitHub가 생성한 SHA-256을 검증합니다.
